...
function App() {
            ...
            const handleAuthCallbackPKCE = async (data: { code: string }) => {
                ...
                try {
                    ...
                    if (error) {
                        ...
                        console.log(
                            "Production PKCE: Session exchanged successfully"
                        );
                    }
                } catch (err) {
                    console.error(
                        "Production PKCE: Error in auth callback:",
                        err
                    );
                }
            };
    ...
    useEffect(() => {
        const initializeAndSubscribe = async () => {
            ...
            const unsubscribeSolutionSuccess =
                window.electronAPI.onSolutionSuccess(async () => {
                    // Wait for initialization before proceeding
                    if (!isInitialized) {
                        console.warn(
                            "Attempted to decrement credits before initialization"
                        );
                        return;
                    }
            ...
            .on(
                ...
                async (payload) => {
                    ...
                    if (
                        ...
                        console.log(
                            "Checking current subscription and credits status..."
                        );
                        const { data: subscription } = await supabase
                            .from("subscriptions")
                            .select("*, credits, preferred_language")
                        ...
                        console.log(
                            "Current subscription check result:",
                            subscription
                        );
                        setIsSubscribed(!!subscription);
                        setCredits(subscription?.credits ?? 0);
