...
export interface IIpcHandlerDeps {
  ...
  increaseOpacity: () => void;
  quitApp: () => void;
  setScreenCaptureProtection: (enableProtection: boolean) => void;
}
...
// Register the crack-coding-interview protocol
if (process.platform === "darwin") {
  app.setAsDefaultProtocolClient("crack-coding-interview");
} else {
  app.setAsDefaultProtocolClient("crack-coding-interview", process.execPath, [
...
async function createWindow(): Promise<void> {
  ...
  // Commenting DevTools temporarily
  if (isDev) {
...
function setScreenCaptureProtection(enableProtection: boolean): void {
  // Enhanced screen capture resistance
  state.mainWindow.setContentProtection(enableProtection);
  state.enableProtection = enableProtection;
  ...
  if (process.platform === "darwin") {
    // Prevent window from being captured in screenshots
    state.mainWindow.setHiddenInMissionControl(enableProtection);
  ...
  // Prevent the window from being captured by screen recording
  state.mainWindow.webContents.setBackgroundThrottling(!enableProtection);
  state.mainWindow.webContents.setFrameRate(60);
}
...
function showMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    state.mainWindow.setContentProtection(state.enableProtection);
    state.mainWindow.setOpacity(0);
...
app.on("open-url", (event, url) => {
  ...
  if (url.startsWith("crack-coding-interview://")) {
...
app.on("second-instance", (event, commandLine) => {
  ...
  const url = commandLine.find((arg) =>
    arg.startsWith("crack-coding-interview://")
  );
  if (url) {
    handleAuthCallback(url, state.mainWindow);
  }
